...
export function GlassCard({ children, className = "", onClick, dark }: {
  ...
  return (
    <div onClick={onClick} className={`rounded-3xl border backdrop-blur-2xl transition-all duration-300 ${
      dark
...
export function GradientButton({ children, className = "", type = "button", disabled = false, onClick, variant = "primary", dark }: {
  ...
  const base = "inline-flex items-center justify-center gap-2 font-semibold rounded-2xl transition-all duration-300 active:scale-[0.97] disabled:opacity-50 disabled:pointer-events-none cursor-pointer";
  const variants: Record<string, string> = {
    primary: "px-6 py-3 bg-gradient-to-r from-rose-500 via-orange-500 to-amber-500 text-white shadow-lg shadow-orange-500/25 hover:shadow-xl hover:shadow-orange-500/35 hover:scale-[1.02]",
    ghost: dark
      ? "px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.08] text-gray-300 border border-white/[0.06]"
      : "px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-200",
    danger: dark
      ? "px-4 py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20"
      : "px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200",
    outline: dark
      ? "px-4 py-2.5 bg-transparent hover:bg-white/[0.04] text-gray-400 border border-white/[0.1]"
      : "px-4 py-2.5 bg-transparent hover:bg-gray-50 text-gray-500 border border-gray-200",
  };
  return (
...
router.push("/auth/login");
      ...
      <div className={`min-h-screen flex items-center justify-center relative overflow-hidden ${darkMode ? "bg-[#06060a]" : "bg-gray-50"}`}>
        ...
        <div className="flex flex-col items-center gap-5 z-10">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-rose-500 to-orange-500 flex items-center justify-center shadow-2xl shadow-rose-500/30 animate-pulse">
              <img src="/logo.png" alt="PawMatch Logo" className="w-14 h-auto" />
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`font-medium tracking-wide ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Loading your dashboard…</span>
          </div>
        </div>
      </div>
    );
  ...
  const stats = [
    { icon: <Heart className="w-5 h-5 text-white" />, label: "Favorites", value: favorites.length, gradient: "from-rose-500 to-pink-600", glow: "shadow-rose-500/20" },
    { icon: <FileText className="w-5 h-5 text-white" />, label: "Applications", value: applications.length, gradient: "from-violet-500 to-purple-600", glow: "shadow-violet-500/20" },
    { icon: <MessageCircle className="w-5 h-5 text-white" />, label: "Messages", value: messages.length, gradient: "from-sky-500 to-blue-600", glow: "shadow-sky-500/20" },
  ];
  ...
  function renderSidebar() {
    return (
      <>
        {/* User Mini Profile */}
        <div className={`p-4 mb-3 rounded-2xl border ${
          d ? "bg-white/[0.03] border-white/[0.04]" : "bg-gray-50 border-gray-100"
        }`}>
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-rose-500 to-orange-500 flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-rose-500/20">
                {user!.name.charAt(0).toUpperCase()}
              </div>
              {user!.isVerified && (
                <div className={`absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 rounded-lg flex items-center justify-center border-2 ${d ? "border-[#0a0a0f]" : "border-white"}`}>
                  <Check className="w-3 h-3 text-white" />
                </div>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <p className={`font-bold truncate ${d ? "text-white" : "text-gray-900"}`}>{user!.name}</p>
              <div className="flex items-center gap-1.5">
                <Shield className="w-3 h-3 text-emerald-400" />
                <span className={`text-xs ${d ? "text-gray-500" : "text-gray-500"}`}>{user!.isVerified ? "Verified Adopter" : "Adopter"}</span>
              </div>
            </div>
          </div>
          {user!.city && (
            <div className={`flex items-center gap-1.5 mt-3 text-xs ${d ? "text-gray-500" : "text-gray-500"}`}>
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <Link key={item.id} href={item.href}
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-left font-medium transition-all duration-300 group relative overflow-hidden ${
                  isActive
                    ? d ? "text-white" : "text-gray-900"
                    : d ? "text-gray-500 hover:text-gray-300 hover:bg-white/[0.03]" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"
                }`}>
                {isActive && (
                  <div className={`absolute inset-0 bg-gradient-to-r ${item.gradient} ${d ? "opacity-[0.12]" : "opacity-[0.08]"} rounded-2xl`} />
                )}
                {isActive && (
                ...
                {item.count > 0 && (
                  <span className={`relative min-w-[24px] h-6 px-2 rounded-full text-[11px] font-bold flex items-center justify-center ${
                    isActive
                ...
                {isActive && <ChevronRight className={`w-4 h-4 relative ${d ? "text-white/50" : "text-gray-400"}`} />}
              </Link>
            );
          })}
        </nav>
    ...
    <DashboardContext.Provider value={{
      ...
      <div className={`min-h-screen relative overflow-hidden transition-colors duration-500 ${
        ...
        <div className="fixed inset-0 -z-10 pointer-events-none">
          <FloatingOrb dark={d} className="w-[700px] h-[700px] bg-rose-600/[0.04] top-[-15%] left-[-10%] animate-pulse" />
          <FloatingOrb dark={d} className="w-[500px] h-[500px] bg-orange-600/[0.03] bottom-[-15%] right-[-10%] animate-pulse" />
          <FloatingOrb dark={d} className="w-[400px] h-[400px] bg-violet-600/[0.03] top-[40%] left-[50%] animate-pulse" />
          {d && (
            <>
        ...
        {mobileSidebar && (
          <div className="fixed inset-0 z-[60] lg:hidden">
            ...
            <div className={`absolute left-0 top-0 bottom-0 w-80 backdrop-blur-2xl border-r p-6 ${
              d ? "bg-[#0a0a0f]/95 border-white/[0.06]" : "bg-white/95 border-gray-200"
            }`} style={{ animation: "slideInLeft 0.3s ease-out" }}>
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  <img src="/logo.png" alt="PawMatch Logo" className="h-10 w-auto" />
                </div>
          ...
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <div className="relative group">
                  <img src="/logo.png" alt="PawMatch Logo" className="h-10 w-auto transition-transform duration-300 group-hover:scale-110" />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/dashboard/messages"
                className={`relative p-2.5 rounded-xl transition-all ${d ? "hover:bg-white/5 text-gray-400 hover:text-gray-200" : "hover:bg-gray-100 text-gray-500 hover:text-gray-700"}`}>
                <Bell className="w-5 h-5" />
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-gradient-to-r from-rose-500 to-orange-500 text-white text-[10px] rounded-full flex items-center justify-center font-bold shadow-lg shadow-rose-500/40 animate-bounce" style={{ animationDuration: "2s" }}>
                    {unreadCount}
                  </span>
                )}
              </Link>
              ...
              <button onClick={handleLogout}
                className={`p-2.5 rounded-xl transition-all ${d ? "hover:bg-red-500/10 text-gray-500 hover:text-red-400" : "hover:bg-red-50 text-gray-400 hover:text-red-500"}`}
                title="Logout">
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 py-6">
          {/* ─── Welcome Banner ────────────────────────────────────────── */}
          <div className="mb-8">
            <div className={`relative overflow-hidden rounded-3xl p-6 md:p-8 border ${d ? "border-white/[0.06]" : "border-gray-200/60"}`}
              style={{
                background: d
              ...
              <div className="absolute top-0 right-0 w-80 h-80 bg-gradient-to-br from-rose-500/10 to-transparent rounded-full -translate-y-1/2 translate-x-1/4 blur-3xl" />
              <div className="absolute bottom-0 left-0 w-60 h-60 bg-gradient-to-tr from-amber-500/10 to-transparent rounded-full translate-y-1/2 -translate-x-1/4 blur-3xl" />
              <div className="absolute top-6 right-8 w-2 h-2 bg-rose-400/40 rounded-full animate-ping" style={{ animationDuration: "3s" }} />
              <div className="absolute bottom-8 right-24 w-1.5 h-1.5 bg-amber-400/40 rounded-full animate-ping" style={{ animationDuration: "4s", animationDelay: "1s" }} />
              <div className="absolute top-12 right-32 w-1 h-1 bg-orange-400/40 rounded-full animate-ping" style={{ animationDuration: "5s", animationDelay: "2s" }} />
              ...
              <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border ${
                      ...
                      <span className={`text-xs font-medium ${d ? "text-amber-400/80" : "text-amber-600"}`}>Good to see you!</span>
                    </div>
                    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border ${
                      d ? "bg-emerald-500/10 border-emerald-500/20" : "bg-emerald-50 border-emerald-200"
                    }`}>
                      <Activity className="w-3.5 h-3.5 text-emerald-400" />
                      <span className={`text-xs font-medium ${d ? "text-emerald-400" : "text-emerald-600"}`}>Active</span>
                    </div>
                  </div>
                  <h1 className={`text-3xl md:text-4xl font-extrabold mb-2 tracking-tight ${d ? "text-white" : "text-gray-900"}`}>
                    Welcome back, {user.name.split(" ")[0]}!{" "}
                    <span className="inline-block animate-bounce" style={{ animationDuration: "2s" }}>🐾</span>
                  </h1>
                  <p className={`text-base max-w-lg leading-relaxed ${d ? "text-gray-400" : "text-gray-600"}`}>
                ...
                <div className="flex gap-3 flex-shrink-0 flex-wrap">
                  <GradientButton dark={d} variant="ghost" onClick={() => router.push("/pets")}>
                    <Search className="w-4 h-4" /> Browse Pets
                  </GradientButton>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {stats.map((stat, i) => (
              <div key={stat.label} className="group cursor-pointer"
                ...
                <GlassCard dark={d} className={`p-5 transition-all duration-500 hover:scale-[1.03] ${
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <p className={`text-[11px] font-semibold uppercase tracking-wider ${d ? "text-gray-500" : "text-gray-400"}`}>{stat.label}</p>
                    <div className={`w-10 h-10 rounded-2xl bg-gradient-to-br ${stat.gradient} flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}>
                      {stat.icon}
                    </div>
                  </div>
                  <p className={`text-3xl font-extrabold tracking-tight ${d ? "text-white" : "text-gray-900"}`}>{stat.value}</p>
